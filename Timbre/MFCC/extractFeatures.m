...
% Output:
%   featureVect: (80 x numBlocks) float matrix, where numBlocks is 
...
featureVect = FeatureSpectralMfccs(xb, fs);

diff_featureVect = diff(featureVect, 1, 2);
diff_featureVect = [zeros(size(featureVect,1), 1), diff_featureVect];

featureVect = [featureVect; diff_featureVect];
